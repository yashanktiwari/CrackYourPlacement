/*
Do simple BFS and color all the connected nodes which have the same initial color
Maintain a visited array to not fall in the infinite loop.

TC -> O(n*m)
SC -> O(n*m) (visited array) + stack space (if using recursion)
*/
class Solution {
